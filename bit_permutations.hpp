...
#if defined(CXX26_BIT_PERMUTATIONS_X86_POPCNT) || defined(CXX26_BIT_PERMUTATIONS_X86_BMI2) || defined(CXX26_BIT_PERMUTATIONS_ARM_SVE)
...
namespace cxx26bp {
    ...
    namespace detail {
        ...
        template <typename T>
        concept signed_or_unsigned_integer = standard_integer<T> || bit_precise_integer<T> || extended_integer<T>;
    ...
    namespace detail {
        ...
        inline constexpr auto alternating_bit_mask_table = [] {
            ...
            for (int i = 0; i <= log_N; ++i) {
                result[static_cast<std::size_t>(i)] = i == log_N ? static_cast<T>(-1) : alternate01<T>((1 << i), (1 << i));
            }
            return result;
        }();
    ...
    [[nodiscard]] constexpr int countr_zero(T x) noexcept {
        ...
        if constexpr (N > N_nat) {
            ...
            constexpr auto sentinel = static_cast<std::size_t>(N % N_nat == 0 ? 0 : std::size_t{1} << (N % N_nat));
            return result + countr_zero(static_cast<std::size_t>(static_cast<std::size_t>(x) | sentinel));
        }
    ...
    [[nodiscard]] constexpr T bit_compressr(T x, T m) noexcept {
        ...
        if constexpr (N > N_native) {
            ...
            for (int mask_bits = 0; mask_bits < N; mask_bits += N_native) {
                const auto compressed = bit_compressr(static_cast<std::size_t>(x), static_cast<std::size_t>(m));
    ...
    [[nodiscard]] constexpr T bit_expandr(T x, T m) noexcept {
        ...
        if constexpr (N > N_native) {
            ...
            CXX26_BIT_PERMUTATIONS_AGGRESSIVE_UNROLL
            for (int mask_bits = 0; mask_bits < N; mask_bits += N_native) {
                const auto expanded = bit_expandr(static_cast<std::size_t>(x), static_cast<std::size_t>(m));
