...
namespace std {
	...
	constexpr const T& get(const ::reachability::details::tuple_leaf<I, T>& t) {
		return t.data;
	}

	template <typename T, std::size_t I>
	constexpr T& get(::reachability::details::tuple_leaf<I, T>& t) {
		return t.data;
	}
...
namespace reachability {
	namespace details {
		...
		template <class... Us>
		tuple_impl(Us...) -> tuple_impl<std::make_index_sequence<sizeof...(Us)>, Us...>;

		template <std::size_t I, typename T>
		T tuple_element_tag(const tuple_leaf<I, T>&);
	} // namespace details
...
namespace std {
	...
	struct tuple_element<I, ::reachability::tuple<Ts...>> {
		using type = decltype(tuple_element_tag<I>(std::declval<::reachability::tuple<Ts...>>()));
	};

	template <class... Ts>
	struct tuple_size<::reachability::tuple<Ts...>> : std::integral_constant<std::size_t, sizeof...(Ts)> {};
} // namespace std
	...
	constexpr auto vec_of = []<class T> {
		if constexpr (!requires { std::tuple_size_v<T>; }) {
			...
			if constexpr (std::is_integral_v<decltype(count_or_f)>) {
				...
				return all_of.template operator()<count_or_f>();
		}
	};
