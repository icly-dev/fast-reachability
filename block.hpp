...
namespace reachability {
	...
	[[gnu::always_inline]] constexpr auto call_with_block(block_type b, auto f) {
		...
		switch (b) {
			case T:
				return f.template operator()<RS::T>();
			case Z:
				return f.template operator()<RS::Z>();
			case S:
				return f.template operator()<RS::S>();
			case J:
				return f.template operator()<RS::J>();
			case L:
				return f.template operator()<RS::L>();
			case O:
				return f.template operator()<RS::O>();
			case I:
				return f.template operator()<RS::I>();
			default:
				std::unreachable();
		}
	}
